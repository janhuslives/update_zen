...
```
cmd_interactive()  (line 8219)             ← entry point, parallel status fetch
  │                                          prompt: [1-n], u=update all, s=settings, f=files, r=reload, q=quit
  ├── u  _batch_update()  (8171)             confirm → update flagged containers sequentially
  │
  ├── f  _interactive_file_manager()  (7524)
  ...
  │
  ├── [1-n] container → _action_menu()  (6655)
  │     │                                    prompt: [u/s/v/x/i/c], 0 back, q quit
  │     │
  │     ├── u  _updates_menu()  (6169)
  │     │     ├── 1  Engine.update()
  ...
  │     │     ├── m  _interactive_container_chmod()  (5945)
  │     │     └── f  browse snapshot folder                           (_browse_path at container snapshot dir; _file_action_menu on file select)
  │     │
  │     ├── v  _interactive_volumes_menu()  (5182)
  │     │     ├── [1-n] mount → _interactive_mount_action()  (4872)
  │     │     │         ├── 1    Skip / Unskip
  ...
  │     │     ├── e → _interactive_volume_encryption_menu()  (5088)   (1-N toggle, a=all, n=none)
  │     │     ├── x → _interactive_exclude_patterns()  (4812)
  ...
  │     │
  │     ├── x  _container_control_menu()  (6366)
  ...
  │     │
  │     └── c  _container_config_menu()  (6051)
  │           ├── e   toggle encrypt_containers  (this container)
  │           ├── k*  _interactive_password_menu()  (6740)            (* only shown when encryption on)
  │           ├── g   _interactive_registries_menu()  (5398)
  │           ├── p   set or clear pinned tag                         (set/clear/cancel sub-prompt when pinned)
  │           ├── b   _backup_toggles_menu()  (6593)
  ...
  │
  └── [s] settings → _interactive_settings()  (8103)
        │                                       prompt: [s/v/a/e/j/d/c/m], 0 back, q quit
        │
        ├── s  _settings_snapshots_menu()  (7814)
        │     ├── 1  global snapshot directory                        (_browse_path → config.snapshot_dir)
        │     ├── 2  global rotation limit                            (prompt → config.max_snapshots)
        │     ├── 3  snapshot permissions                             (_interactive_global_chmod  (7573) + optional sweep)
        │     └── 4  toggle image_export_enabled
        │
        ├── v  _settings_volumes_menu()  (7871)
        │     ├── 1  toggle volume_backup_enabled
        │     └── 2  toggle pause_for_backup  (global)
        │
        ├── a  toggle auto_rollback  (global; immediate, no submenu)
        │
        ├── e  _interactive_encryption_menu()  (6921)
        │     ├── 1  _interactive_saved_passwords_menu()  (6833)      — list/add/delete/purge
        │     ├── 2  mode selector  (session / always / saved)
        │     └── 3  per-container status  (read-only)
        │
        ├── j  _interactive_cron_overview()  (5891)                   (global read-only job summary + apply)
        │
        ├── d  _settings_display_menu()  (7907)
        │     ├── 1  _interactive_column_menu()  (7738)               (N of 21 optional columns shown)
        │     └── 2  toggle pagination_enabled
        │
        ├── c  _settings_config_files_menu()  (7941)
        ...
        │
        └── m  _settings_maintenance_menu()  (8014)
